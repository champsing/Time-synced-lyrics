...
import { INSTRUMENTAL, LIVE, ORIGINAL, THE_FIRST_TAKE } from "@/composables/utils/config";
...
const parseSubtitle = (subtitle: string) => subtitle?.replace(/\\n/g, "\n") || "";
</script>

<template>
  <div class="song-info w-full max-w-95 lg:max-w-110 md:pt-10">
    <h1 class="text-white text-2xl lg:text-3xl font-bold tracking-tight leading-tight line-clamp-2">
    ...
    <div class="flex items-center gap-4 mt-2 flex-wrap">
      ...
      <span
        ...
        :class="{
          'bg-cyan-500/20 text-cyan-400 border-cyan-500/30': songVersion === INSTRUMENTAL,
          'bg-white/10 text-white border-white/20': songVersion === THE_FIRST_TAKE,
          'bg-rose-500/20 text-rose-400 border-rose-500/30': songVersion === LIVE,
        }"
      >
        {{
    ...
    <p v-if="currentSong.subtitle" class="text-white/50 text-xs mt-2 line-clamp-2 italic">
